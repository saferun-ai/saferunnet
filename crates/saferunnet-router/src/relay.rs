...
use saferunnet_exit::ExitPolicy;
use saferunnet_link::{FrameKind, LlarpFrame};
use saferunnet_service::AuthenticatedTransitHopMessage;
use thiserror::Error;
        ...
        match frame.kind {
            ...
            FrameKind::RelayData => {
                ...
                if self.is_exit_hop(frame, total_hops)
                    && let Ok((target, port)) = saferunnet_exit::parse_exit_target(&inner)
                {
...
impl From<saferunnet_service::TransitHopError> for RelayError {
    fn from(e: saferunnet_service::TransitHopError) -> Self {
