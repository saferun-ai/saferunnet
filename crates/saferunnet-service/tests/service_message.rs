...
#[test]
fn link_session_init_service_kind_round_trip_preserves_kind() {
    let identity = make_identity("alice");
    let message = AuthenticatedServiceMessage::sign(
        &identity,
        ServiceMessageKind::LinkSessionInit,
        b"session-ready".to_vec(),
    )
    .expect("sign should succeed");
    ...
    assert_eq!(decoded.kind(), ServiceMessageKind::LinkSessionInit);
    assert_eq!(decoded.body(), b"session-ready");
    decoded.verify().expect("decoded message should verify");
}

#[test]
fn link_session_accept_service_kind_round_trip_preserves_kind() {
    let identity = make_identity("alice");
    let message = AuthenticatedServiceMessage::sign(
        &identity,
        ServiceMessageKind::LinkSessionAccept,
        b"session-accepted".to_vec(),
    )
    .expect("sign should succeed");
    ...
    assert_eq!(decoded.kind(), ServiceMessageKind::LinkSessionAccept);
    assert_eq!(decoded.body(), b"session-accepted");
    decoded.verify().expect("decoded message should verify");
}
...
#[test]
fn link_session_close_service_kind_round_trip_preserves_kind() {
    let identity = make_identity("alice");
    let message = AuthenticatedServiceMessage::sign(
        &identity,
        ServiceMessageKind::LinkSessionClose,
        b"session-closed".to_vec(),
    )
    .expect("sign should succeed");
    ...
    assert_eq!(decoded.kind(), ServiceMessageKind::LinkSessionClose);
    assert_eq!(decoded.body(), b"session-closed");
    decoded.verify().expect("decoded message should verify");
}
