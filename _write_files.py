...
LIB_RS = r"D:\Projects\RustProjects\ReburnSaferunNet\crates\saferunnet-service\src\lib.rs"
TRANSIT_HOP_RS = r"D:\Projects\RustProjects\ReburnSaferunNet\crates\saferunnet-service\src\transit_hop.rs"
TEST_RS = r"D:\Projects\RustProjects\ReburnSaferunNet\crates\saferunnet-service\tests\transit_hop.rs"

lib_rs = """use saferunnet_crypto::{EnvelopeCodecError, SignatureError, SignedEnvelope, SignedEnvelopeCodec};
...
pub enum ServiceMessageError {
    ...
    #[error("service message `{field}` frame component exceeds encoded limit `{max}` with length `{length}` bytes")]
    FrameLengthOverflow {
        field: &'static str,
        length: usize,
        max: usize,
...
with open(LIB_RS, 'w', encoding='utf-8') as f:
    f.write(lib_rs)
print("lib.rs written successfully")
